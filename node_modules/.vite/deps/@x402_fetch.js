...
ZodPipeline.create;
//#endregion
//#region node_modules/@x402/core/dist/esm/chunk-FPXAE3OS.mjs
...
var ResourceInfoSchema = objectType({
	...
	serviceName: stringType().min(1).max(32).regex(PRINTABLE_ASCII_REGEX).optional(),
	tags: arrayType(stringType().min(1).max(32).regex(PRINTABLE_ASCII_REGEX)).max(5).optional(),
	iconUrl: stringType().max(2048).optional()
});
...
//#endregion
//#region node_modules/@x402/core/dist/esm/chunk-ABS7D6VX.mjs
...
var x402HTTPClient = class {
	...
	*/
	async processPaymentResult(paymentPayload, getHeader, status) {
		...
		let paymentRequired;
		if (!settleResponse && status === 402) try {
			paymentRequired = this.getPaymentRequiredResponse(getHeader);
		} catch {}
		const requirements = paymentPayload.accepted;
		if (!requirements) throw new Error("Invalid x402 v2 payment payload: missing `accepted`");
		const ctx = {
			paymentPayload,
			requirements,
		...
		return {
			recovered: (await this.client.handlePaymentResponse(ctx))?.recovered === true,
			settleResponse
		};
	}
	/**
	...
	*/
	parsePaymentResult(args) {
		...
		if (header && !("success" in header)) paymentStatus = "payment_required";
		if (header && "success" in header) paymentStatus = header.success ? "settled" : "settle_failed";
		return {
			status,
			paymentStatus,
			body,
	...
	*/
	getPaymentRequiredHooks(paymentRequired) {
		...
		for (const extension of this.client.getExtensions()) {
			const hook = extension.transportHooks?.http?.onPaymentRequired;
...
//#endregion
//#region node_modules/@x402/core/dist/esm/client/index.mjs
var x402Client = class _x402Client {
	...
	*/
	async handlePaymentResponse(ctx) {
		for (const hook of this.getLabeledHooks("onPaymentResponse", ctx.paymentPayload.x402Version, ctx.requirements, ctx.paymentRequired?.extensions ?? ctx.paymentPayload.extensions)) {
			...
			if (result && "recovered" in result && result.recovered) return { recovered: true };
		}
	}
	/**
	...
	*/
	async createPaymentPayload(paymentRequired) {
		...
		try {
			...
			for (const hook of this.getLabeledHooks("afterPaymentCreation", paymentRequired.x402Version, requirements, paymentRequired.extensions)) await hook(createdContext);
	...
	*/
	mergeExtensions(serverExtensions, clientExtensions) {
		...
		for (const [key, clientValue] of Object.entries(clientExtensions)) {
			...
			const pending = [{
				target: extensionValue,
				source: clientRecord
			}];
			for (const item of pending) for (const [fieldKey, clientFieldValue] of Object.entries(item.source)) {
				...
				if (!Object.prototype.hasOwnProperty.call(item.target, fieldKey)) item.target[fieldKey] = clientFieldValue;
			}
	...
	*/
	_registerScheme(x402Version2, network, client) {
		...
		clientSchemesByNetwork.get(network).set(client.scheme, client);
		if (!this.schemeClientHookAdapters.has(x402Version2)) this.schemeClientHookAdapters.set(x402Version2, /* @__PURE__ */ new Map());
		const adaptersByNetwork = this.schemeClientHookAdapters.get(x402Version2);
		if (!adaptersByNetwork.has(network)) adaptersByNetwork.set(network, /* @__PURE__ */ new Map());
		...
		const handles = {};
		if (hooks.onBeforePaymentCreation) handles.beforePaymentCreation = hooks.onBeforePaymentCreation;
		if (hooks.onAfterPaymentCreation) handles.afterPaymentCreation = hooks.onAfterPaymentCreation;
		if (hooks.onPaymentCreationFailure) handles.onPaymentCreationFailure = hooks.onPaymentCreationFailure;
		if (hooks.onPaymentResponse) handles.onPaymentResponse = hooks.onPaymentResponse;
		if (Object.keys(handles).length > 0) adaptersByScheme.set(client.scheme, handles);
		else adaptersByScheme.delete(client.scheme);
		return this;
	}
	/**
	...
	*/
	getLabeledHooks(phase, x402Version2, requirements, declaredExtensions) {
		...
		const out = [...manual];
		const adaptersByNetwork = this.schemeClientHookAdapters.get(x402Version2);
		const hook = (adaptersByNetwork ? findByNetworkAndScheme(adaptersByNetwork, requirements.scheme, requirements.network) : void 0)?.[phase];
		if (hook !== void 0) out.push(hook);
		if (!declaredExtensions) return out;
	...
	*/
	getClientExtensionHookKey(phase) {
		switch (phase) {
			case "beforePaymentCreation": return "onBeforePaymentCreation";
			case "afterPaymentCreation": return "onAfterPaymentCreation";
			case "onPaymentCreationFailure": return "onPaymentCreationFailure";
			case "onPaymentResponse": return "onPaymentResponse";
		}
	}
};
//#endregion
//#region node_modules/@x402/fetch/dist/esm/index.mjs
function wrapFetchWithPayment(fetch, client) {
	...
	return async (input, init) => {
		...
		if (hookHeaders) {
			...
			for (const [key, value] of Object.entries(hookHeaders)) hookRequest.headers.set(key, value);
		...
		if (clonedRequest.headers.has("PAYMENT-SIGNATURE") || clonedRequest.headers.has("X-PAYMENT")) throw new Error("Payment already attempted");
		for (const [key, value] of Object.entries(paymentHeaders)) clonedRequest.headers.set(key, value);
		...
		if ((await httpClient.processPaymentResult(paymentPayload, (name) => secondResponse.headers.get(name), secondResponse.status)).recovered) {
			...
			for (const [k, v] of Object.entries(retryHeaders)) retryRequest.headers.set(k, v);
...
function wrapFetchWithPaymentFromConfig(fetch, config) {
	return wrapFetchWithPayment(fetch, x402Client.fromConfig(config));
}
//#endregion
export { decodePaymentResponseHeader, wrapFetchWithPayment, wrapFetchWithPaymentFromConfig, x402Client, x402HTTPClient };
